...
.agent-policy/
├── policy.yaml
├── policy.lock.json       # integrity record when generated by the consumer workflow
├── invariants.yaml
├── overlays/rules.yaml
...
Drift is never silently adopted. The reconciliation choices are:

- `adopt` — propose a canonical-source change only when the artifact edit is representable as canonical source intent; the proposal still requires a fresh reviewed plan;
- `regenerate` — discard artifact edits and create a fresh plan from canonical sources; the new output still requires review;
- `abort` — write nothing.

Non-interactive drift remains unresolved and fails. A manual edit to a generated skill, an unmanaged edit around a Managed Region, a foreign owner, duplicate markers, or a changed source hash must be resolved before application.
