...
import type { ReconciliationChoice } from '../arguments.js'
...
import type { ArtifactDrift } from '../../applier/reconcile.js'
...
export class ArtifactDriftError extends Error implements ArtifactDrift {
  readonly artifactPath: string
  readonly currentContent: string

  constructor(artifactPath: string, currentContent: string, message = `Generated artifact drift at ${artifactPath}`) {
    super(message)
...
export async function chooseReconciliation(
  args: { readonly reconcile?: ReconciliationChoice },
  io: CliIo,
  message: string,
): Promise<ReconciliationChoice | undefined> {
  if (args.reconcile !== undefined) return args.reconcile
  return io.choose === undefined ? undefined : io.choose(message, ['adopt', 'regenerate', 'abort'])
}

export function reconciliationPlanPath(path: string): string {
  return `${resolve(path)}.regenerated.json`
}

/** Turn a drifted plan into a fresh external regeneration plan without applying anything. */
export async function saveRegenerationPlan(
  context: CommandContext,
  original: ChangePlan,
  planPath: string,
  includeReviewedSourceChanges = true,
): Promise<ChangePlan> {
  const sourceChanges = includeReviewedSourceChanges ? (original.sourceChanges ?? []) : []
  const sourceOverrides = new Map(sourceChanges.map(({ path, content }) => [path, content]))
  const compilation = await compileCodex(context, undefined, sourceOverrides)
  return saveProjectionPlan(
    context,
    `${original.command}:regenerate`,
    planPath,
...
export function generatedOwnership(content: string): boolean {
  ...
  try {
    const parsed = JSON.parse(stable) as { readonly generatedBy?: unknown }
    return parsed.generatedBy === toolkitOwner
  } catch {
    return false
  }
}
...
function replaceSequenceField(source: string, field: 'bundles' | 'targets', selected: readonly string[]): string | undefined {
  ...
  const pair = document.contents.items.find((candidate) =>
    isPair(candidate) && isScalar(candidate.key) && candidate.key.value === field,
  )
...
export async function prepareBundleSelection(
  ...
  const selectedTargets = requiredTarget === undefined || source.targets.includes(requiredTarget)
    ? source.targets
    : [...source.targets, requiredTarget]
  if (
    JSON.stringify(source.bundles) === JSON.stringify(selected)
    && JSON.stringify(source.targets) === JSON.stringify(selectedTargets)
  ) {
  ...
  if (JSON.stringify(source.bundles) !== JSON.stringify(selected)) {
    content = replaceSequenceField(content, 'bundles', selected) ?? undefined
    if (content === undefined) {
      throw policyError('MISSING_BUNDLE_SELECTION', `Canonical source has no bundles field: ${source.path}`, source.path)
    }
  }
  if (JSON.stringify(source.targets) !== JSON.stringify(selectedTargets)) {
    content = replaceSequenceField(content, 'targets', selectedTargets) ?? undefined
    if (content === undefined) {
      throw policyError('MISSING_TARGET_SELECTION', `Canonical source has no targets field: ${source.path}`, source.path)
    }
  }
  return {
    sourceChanges: [{
...
function policyWithManifestOverride(
  source: ProjectPolicySource,
  manifestSource: string,
): ProjectPolicySource {
  const parsed = validateDocument<ProjectPolicy & { readonly overlays?: readonly string[] }>(
    'project-policy-v1',
    parseYamlDocument(manifestSource, source.path),
    source.path,
  )
  const { overlays: _overlays, ...policy } = parsed
...
export async function compileCodex(
  ...
  if (!project.targets.includes('codex')) {
    throw policyError(
      'TARGET_NOT_SELECTED',
      'Codex is not selected in .agent-policy/policy.yaml targets',
      project.path,
    )
  }
  ...
  return {
    ...
    sourcePaths,
    canonicalSourceHash,
    artifacts: [...projectedArtifacts, lock],
  }
}
