...
export async function stageSourceChange(request: StageSourceRequest): Promise<ChangePlan> {
  ...
  if (
    ...
    throw policyError("PATH_ESCAPES_PROJECT", `Declared path escapes project: ${targetPath}`, targetPath)
  }

  if (scope === "project") {
    if (!normalized.startsWith(".agent-policy/")) {
      throw policyError(
  ...
  // Schema validation based on target file path
  if (normalized.startsWith(".agent-policy/rules/") && normalized.endsWith(".md")) {
    parseRuleMarkdown(content, normalized)
  } else if (normalized.startsWith(".agent-policy/overlays/") && (normalized.endsWith(".yaml") || normalized.endsWith(".yml"))) {
